...
struct Scanner {
    ...
    void advance(TSLexer *lexer) {
        ...
        if (consumed == '\n') {
            consumeComment(lexer, false);
        }
    }

    // consume all consecutive comment lines
    bool consumeComment(TSLexer *lexer, bool b) {
        ...
        while (!onNewline(lexer) && !lexer->eof(lexer)) {
            lexer->advance(lexer, b);
        }

        // consume the ending newline
        !b ? advance(lexer) : skip(lexer);
    ...
    bool scan(TSLexer *lexer, const bool *valid_symbols) {
        ...
        bool commentFound = consumeComment(lexer, true);
        if (commentFound) {
        ...
        // spaces on the last line = indent information
        ...
        for (;;) {
            if (onNewline(lexer) && newline_count == 0) {
                ...
                if (space_count == indent_level * INDENT_LEN) {
                ...
                if (space_count == (indent_level + 1) * INDENT_LEN) {
        ...
        uint16_t new_indent_level = space_count / INDENT_LEN;
        ...
        if (!lexer->eof(lexer) && !newline_count) {
            bool tokenFound = scanText(lexer, valid_symbols, space_count);
            if (tokenFound) {
                return tokenFound;
            }
        }
                ...
                if (valid_symbols[EMPTY_LINE] || valid_symbols[MULTI_EMPTY_LINE]) {
                    ...
                    if (onNewline(lexer)) {
                        ...
                        while (lexer->lookahead == ' ') {
                            ...
                            advance(lexer);
                            ++next_line_space_count;
                        }

                        new_indent_level = next_line_space_count / INDENT_LEN;

                        // TODO: Should there be >= ? test
                ...
                if (valid_symbols[MULTI_EMPTY_LINE] || valid_symbols[EMPTY_LINE]) {
                    ...
                    for (;;) {
                        if (onNewline(lexer)) {
                            ...
                            advance(lexer);
                            ++space_count;
                            if (space_count == INDENT_LEN * indent_level) {
                                mark_end(lexer);
                            }
                    ...
                    new_indent_level = space_count / INDENT_LEN;
                    if (newline_count >= 2) {
    ...
    bool scanText(TSLexer *lexer, const bool *valid_symbols, int spaces_consumed) {

        if (lexer->lookahead == '.' && get_column(lexer) == 0) {
            // the first character on a line of text can not be a dot
            // see the docpart/block ambiguity
            // warning: it is possible that this is too much of a quickfix, consult with the founder
            return false;
        }
        ...
        if (valid_symbols[TEXT] || valid_symbols[TEXT_NO_SPACE_NO_DOT] || valid_symbols[VERBOSE_INNER_ENV_META]) {
            int spaceDotCount = spaces_consumed;
            int consumed = spaces_consumed;
    ...
    bool scanFragile(TSLexer *lexer, const bool *valid_symbols) {
        ...
        while (true) {
            if (prev_newline && (onNewline(lexer) || lexer->eof(lexer))) {
                break;
            } else if (onNewline(lexer)) {
                prev_newline = true;
            } else {
                prev_newline = false;
            }
